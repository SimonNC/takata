...
function checkUserInput(userInput) {
  // Expression régulière pour vérifier le format du VIN
  const vinRegex = /^[A-HJ-NPR-Z0-9]{17}$/i;
  // Expression régulière pour vérifier le format de l'immatriculation NC
  const immatriculationNCRegex = /^[0-9]{6}NC$/;
  return vinRegex.test(userInput) || immatriculationNCRegex.test(userInput);
}
...
function LookupForm() {
  ...
  const URL =
    "https://api.citroen-ds-campagne-de-rappel.nc/wp-json/serial-checker/v1/check-serial";
  ...
  async function handleSubmit(e) {
    ...
    if (!vin && checkUserInput(vin)) return;

    try {
      setIsloading(true);
      const newVin = { serial_number: vin.trim() };
    ...
    <div className="bg-transparent border rounded-lg dark:border-gray-700 focus-within:border-primary focus-within:ring focus-within:ring-primary dark:focus-within:border-primary focus-within:ring-opacity-20">
      <form
        ...
        <input
          ...
          className="text-sm lg:text-base font-bold flex-1 px-4 m-1 text-gray-700 placeholder-gray-400 bg-transparent border-none appearance-none h-12 max-w-[75%] lg:max-w-none  focus:outline-none focus:placeholder-transparent focus:ring-0"
          required="required"
          disabled={isLoading}
